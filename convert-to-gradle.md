...
- Gradle wrapper properties, and full wrapper files when `gradle` is available
...
Running the script in this checkout generated:

- `build.gradle`
- `settings.gradle`
- `.gitignore`
- `src/main/java/com/shopopedia/order/OrderServiceApplication.java`
- `src/main/resources/application.yml`
- `src/main/resources/application-oracle.yml`
- `gradle/wrapper/gradle-wrapper.properties`

`gradle wrapper` failed on this machine with a native services error for `libnative-platform.dylib` on Mac OS X aarch64, so `gradlew` and `gradle-wrapper.jar` were not created automatically.
...
6. Creates `src/main/resources/application.yml` with a local H2 datasource so the app starts without Oracle.
7. Creates `src/main/resources/application-oracle.yml` with the Oracle datasource settings.
8. Generates the Gradle wrapper when `gradle` is available.
...
- Run with `--spring.profiles.active=oracle` when you want to use the Oracle datasource.
