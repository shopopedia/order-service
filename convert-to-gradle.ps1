param(
    ...
    [string]$ServerPort = "8085",
    [string]$ApplicationPackage = "",
    [string]$DatasourceUrl = "jdbc:oracle:thin:@localhost:1521/FREEPDB1",
    [string]$DatasourceUsername = "order_service",
    [string]$DatasourcePassword = "order_service_pass",
...
$h2DatabaseName = [regex]::Replace($ProjectName.ToLowerInvariant(), '[^a-z0-9]+', '_').Trim('_')
if ([string]::IsNullOrWhiteSpace($h2DatabaseName)) {
    $h2DatabaseName = "order_service"
}
...
spring:
  ...
  datasource:
    url: jdbc:h2:mem:$h2DatabaseName;MODE=Oracle;DB_CLOSE_DELAY=-1;DB_CLOSE_ON_EXIT=FALSE
    username: sa
    password: ""
...
$applicationOracleYmlPath = Join-Path $mainResourcesDir "application-oracle.yml"
if (-not (Test-Path $applicationOracleYmlPath)) {
    $applicationOracleYml = @"
spring:
  datasource:
    url: $DatasourceUrl
...
"@
    Set-Content -Path $applicationOracleYmlPath -Value $applicationOracleYml -Encoding ascii
}
